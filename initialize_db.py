#!/usr/bin/env python
"""
Initialize Database - Run before app starts
This ensures all tables exist before the app tries to use them
"""
...
print("\n" + "="*70)
print("🔧 Initializing NexiFit Database...")
print("="*70)
...
try:
    ...
    with get_db_connection() as conn:
        ...
        if table_count < 5:
            print(f"⚠️ Only {table_count} tables found - running full setup...")
            ...
            for i, cmd in enumerate(setup_commands, 1):
                try:
                    cursor.execute(cmd)
                    print(f"  ✅ Table {i}/{len(setup_commands)}")
                except Exception as e:
                    if "already exists" in str(e).lower():
                        print(f"  ℹ️ Table {i} already exists")
                    else:
                        print(f"  ⚠️ Table {i}: {e}")
            ...
            indexes = [
                'CREATE INDEX IF NOT EXISTS idx_workout_logs_phone_date ON workout_logs(phone_number, date_completed)',
                'CREATE INDEX IF NOT EXISTS idx_user_profiles_phone ON user_profiles(phone_number)',
                'CREATE INDEX IF NOT EXISTS idx_user_tip_history_phone ON user_tip_history(phone_number)',
                'CREATE INDEX IF NOT EXISTS idx_user_tip_history_date ON user_tip_history(sent_date)',
                'CREATE INDEX IF NOT EXISTS idx_daily_schedule_phone ON daily_workout_schedule(phone_number)',
                'CREATE INDEX IF NOT EXISTS idx_daily_schedule_active ON daily_workout_schedule(active)',
            ]
            
            for idx_cmd in indexes:
                try:
            ...
            try:
                ...
                print(f"✅ Admin user ready: {default_admin}")
            except Exception as e:
            ...
            if tip_count == 0:
                ...
                print(f"✅ Tips seeded: {new_count} total")
            else:
                print(f"✅ Tips already seeded: {tip_count} total")
        else:
            print(f"✅ Database ready: {table_count} tables found")
